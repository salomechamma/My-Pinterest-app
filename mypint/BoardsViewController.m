...
@interface BoardsViewController () <UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"BoardCell" bundle:nil] forCellReuseIdentifier:@"BoardCell"];
    self.tableView.rowHeight = 140.0;
    
    self.navigationItem.title = NSLocalizedString(@"Boards", nil);
    
    __weak BoardsViewController *weakSelf = self;
...
- (void)createPinOnBoard:(PDKBoard *)board
{
    ...
    __weak BoardsViewController *weakSelf = self;
    BoardCell *cell = (BoardCell *)[self.tableView cellForRowAtIndexPath:self.boardIndexPath];
...
{
    PDKBoard *board = self.boards[indexPath.row];
    PinsViewController *pinsVC = [[PinsViewController alloc] initWithBoard:board];
